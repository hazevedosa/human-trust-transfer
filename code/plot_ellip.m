function plot_ellip(mu, covariance, chi)
...
hold on;
axis equal
% axis([-max(r_ellipse(:,1) + X0), max(r_ellipse(:,1) + X0), -max(r_ellipse(:,2) + Y0), max(r_ellipse(:,2) + Y0)])
...
hYLabel = ylabel('$\lambda_2$','interpreter','latex');
